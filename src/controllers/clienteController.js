...
class ClienteController {
    static async listarClientes(req, res) {
        try {
            ...
            res.status(500).json({ message: `${erro.message} - falha na requisição de clientes` });
        }
    }
    ...
    static async listarClientePorId(req, res) {
        try {
            ...
            res.status(500).json({ message: `${erro.message} - falha na requisição de cliente por id` });
        }
    }
    ...
    static async atualizarCliente(req, res) {
        try {
            ...
            res.status(500).json({ message: `${erro.message} - falha na atualização de cliente` });
        }
    }
    ...
    static async removerCliente(req, res) {
        try {
            ...
            res.status(500).json({ message: `${erro.message} - falha na remoção de cliente` });
        }
    }
    ...
    static async atualizarCarrinhoDoClientePorCpf(cpfCliente, carrinho) {
        ...
        await cliente.findByIdAndUpdate(clienteDoCarrinho._id, clienteMaisCarrinho);
    }
    ...
    static async getClientePorIdDoCarrinho(idCarrinho) {
        try {
            const clienteEncontrado = await cliente.findOne({"carrinho._id": idCarrinho})
            return clienteEncontrado
        } catch (erro) {
            console.log(erro.message)
        }
